...
Play.prototype.scheduler = function() {
  while (this.nextNoteTime < this.context.currentTime + this.scheduleAheadTime) {
    this.scheduleNote();
    this.nextNoteTime += 0.016; // 16ms intervals
  }
  setTimeout(() => this.scheduler(), this.lookahead);
};

Play.prototype.scheduleNote = function() {
  if (this.buffer.length === 0) return;

  var note = this.buffer.shift();

  if (note.pitch === -3) {
    // Callback note
    setTimeout(note.callback, 0);
    return;
  }

  if (note.pitch >= 0) {
    // Play a musical note
    var frequency = Math.pow(2, (note.pitch + 3) / 12) * 55;
    this.playTone(frequency, note.len, note.art, this.nextNoteTime);
  } else if (note.pitch === -2) {
    // Rest/pause - just advance time
    this.nextNoteTime += note.len;
  }
};

Play.prototype.playTone = function(frequency, duration, articulation, startTime) {
  ...
  // Create envelope for articulation
  var attackTime = 0.01;
  var releaseTime = duration * (1 - articulation);
  var sustainTime = duration * articulation - attackTime;

  envelope.gain.setValueAtTime(0, startTime);
  envelope.gain.linearRampToValueAtTime(this.vol, startTime + attackTime);
  envelope.gain.setValueAtTime(this.vol, startTime + attackTime + sustainTime);
  ...
  this.nextNoteTime = Math.max(this.nextNoteTime, startTime + duration);
};
// Removed deprecated diffeq function - no longer needed with modern audio implementation
